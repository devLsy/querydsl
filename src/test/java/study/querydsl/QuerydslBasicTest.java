...
//                .fetch();
//        Member fetch = queryFactory.selectFrom(QMember.member).fetchOne();
//
...
//                .leftJoin(member.team, team).on(member.username.eq(team.name))
     ...
     * ex) SELECT ... FROM member WHERE (SELECT max(age) FROM member)
     */
    @Test
    ...
    /**
     * 나이가 평균 이상인 회원 조회(서브 쿼리), in절 이용하기 위한 예제
     */
    @Test
    ...
    /**
     *  동적 쿼리 예제(booleanBuilder 이용)
     */
    @Test
    public void dynamicQuery_BooleanByilder() {
        // usernameParam, ageParam이 있으면 param값 붙여서 조건으로 조회, 없으면 조건 추가 안함
